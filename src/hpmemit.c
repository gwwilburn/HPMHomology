...
int main(int argc, char *argv[]) {
   ...
   if ((ofp = fopen(esl_opt_GetArg(go, 2), "w")) == NULL) esl_fatal("Failed to open output file %s", esl_opt_GetArg(go, 2));

   /* if user has defined an alphabet we define it here */
   if      (esl_opt_GetBoolean(go, "--amino")) abc = esl_alphabet_Create(eslAMINO);
   else if (esl_opt_GetBoolean(go, "--dna"))   abc = esl_alphabet_Create(eslDNA);
   else                                        abc = esl_alphabet_Create(eslRNA);
...
static void emit_alignment(ESL_GETOPTS *go, FILE *ofp, int outfmt, ESL_RANDOMNESS *rng, HPM *hpm, FILE *burninfp, int v)
{
   ...
   P7_TRACE **tr        = NULL;                              /* trace array for emitted seqs  */
   int        N         = esl_opt_GetInteger(go, "-N");      /* number of seqs to emit        */
   int        L         = esl_opt_GetInteger(go, "-L");      /* target sequence length        */
   int        Niter     = esl_opt_GetInteger(go, "--Niter"); /* # of MCMC iterations per seq  */
   int        optflags  = p7_ALL_CONSENSUS_COLS;             /* output MSA options            */
   int        i,j;                                           /* sequence and position indices */
...
int sample_trace(int L, P7_TRACE *tr, ESL_SQ *msq, HPM *hpm, ESL_RANDOMNESS *rng, int v)
{
   ...
   int    showi;                 /* do we emit residue from N/C state                */
