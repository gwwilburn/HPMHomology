...
int main(int argc, char *argv[])  {
   ...
   double      **abc_ct       = NULL;                /* [0..msa->alen-1][0..abc->K] number of each residue at each   *
                                                      * position (abc->K is gap)                                     */
   double    ***bp_ct         = NULL;                /* [0..msa->alen-1][0..msa->alen-1][0..abc->K][0..abc->K]       *
                                                      * number of each combination of residues at all pairs of sites */
   int           use_weights  = 0;                   /* use PB weights when calculating MSA frequencies              */
   int           pk           = 0;                   /* leep track of pseudoknotted basepairs                        */
   ...
   if ( (status = esl_msaweight_PB(msa)) != eslOK) esl_fatal("esl_msaweight_PB() failed, returned status %d\n", status);

   /* trim msa to consensus */
   if ( (status = trim_msa_rf(msa, abc, errbuf)) != eslOK) esl_fatal("trim_msa_rf() failed, returned status %d\n%s\n", status,errbuf);

   /* find annotated basepairs */
   if ( (status = find_basepairs(msa, pk, &ct, errbuf)) != eslOK) esl_fatal("find_basepairs() failed, returned status %d\n%s\n", status, errbuf);

   /* calculate alignment frequencies */
   if ( (status = count_msa(msa, ct, use_weights, &abc_ct, &bp_ct, errbuf)) != eslOK) esl_fatal("count_msa() failed, returned status %d\n%s\n", status, errbuf);

   /* Convert frequencies to Potts model */
   if ( (counts_to_potts(msa, abc_ct, bp_ct, ct, &potts)) != eslOK) esl_fatal("counts_to_potts() failed, returned status %d\n", status);

   /* combine potts and HMM to produce HPM */
   if ( (hpm = hpm_Create_hmm_potts(hmm, potts,abc)) == NULL) esl_fatal("hpm_Create_hmm_potts() failed\n");

   /* open hpm outfile for writing, write hpm to it */
   if ( (hpm_outfp = fopen(hpm_outfile, "w")) == NULL) esl_fatal("Failed to open output hpm file %s for writing\n", hpm_outfile);
   if ( (status = hpmfile_Write(hpm_outfp, hpm)) != eslOK) esl_fatal("hpmfile_Write() failed, returned status %d\n", status);
...
/* Function: trim_msa_rf()
 * Synopsis: Trim <msa> to only its consensus columns
 *
 * args:     msa    - alignment to be trimmed
 *           abc    - alphabet
 *           errbuf - for error messages
 *
 * Returns:  eslOK on success, <msa> is trimmed
 */

static int trim_msa_rf(ESL_MSA *msa, ESL_ALPHABET *abc, char *errbuf) {
   int  status;       /* esl return code                    */
   int  rflen = 0;    /* number of consensus columnn        */
   int  apos  = 0;    /* alignment position index           */
   int *useme = NULL; /* mask for trimming MSA to consensus */
...
/* Function:  find basepairs()
 * Synopsis:  locate the annotated basepairs in <msa>
 *
 * args:     msa    - alignment w/ SS_cons line
 *           pk     - boolean to consider annotated pseudoknots
 *           ret_pk - [1...msa-alen] array, to be returned
 *                    ret_ct[i] = annotated basepair of column i,
 *                    else 0 (CT format)
 *           errbuf - for error messages
 *
 * Returns:  eslOK on success, basepairs in <ret_pk>
 */
...
/* Function: count_msa()
 * Synopsis: count observed residue frequencies in <msa>,
 *           both single site and pairwise for annotated
 *           basepairs
 *
 * args:     msa        - alignment to be analyzed
 *           ct         - CT array with annotated basepairs
 *           useweights - boolean to use seq weights in
 *                        msa->wgt to weight counts
 *           ret_abc_ct - [0...msa->alen-1] array of single-site
 *                        counts,
 *           ret_bp_ct  - [0...msa->alen-1] array pairwise resdue
 *                        counts, only for annotqted bp. For a bp,
 *                        the 5'(left) site contains the counts;
 *                        if i is  unpaired or a 3's site,
 *                        ret_bp_ct is unallocated.
 *
 * Returns:  eslOK on success, counts in <ret_abc_ct> and
 *           <ret_bp_ct>
 */

static int count_msa(ESL_MSA *msa, int *ct, int use_weights, double ***ret_abc_ct, double ****ret_bp_ct, char *errbuf)
{
   double  **abc_ct = NULL; /* single-site residue counts */
   double ***bp_ct  = NULL; /* basepair residue counts    */
...
/* Function:  counts_to_potts()
 * Synopsis:  create <potts> using single site and pairwise residue
 *            conts from <msa>
 *
 * args:     msa       - Training alignment
 *           abc_ct    - Single site residue counts in <msa>
 *           bp_ct     - Pairwise residue count for annotated bp in <msa>
 *           ct        - CT-array marking annotated bp in <msa>
 *           ret_potts - POTTS model to return
 *
 * Returns:  eslOK on success, POTTS model in <ret_potts>
 */
